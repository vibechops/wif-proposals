# Cost breakdown & income model: Dell × WIF India

Internal working doc. Built bottoms-up per program. FX: **$1 ≈ ₹85**. Tax model: **~25.17%** on gross profit.

> Premiere Program costs are **draft estimates** pending Rabia review. Lab and Internship sourced from intake documents.

---

## 1. Executive summary (read this first)

| | Lab | Premiere | Internship (annual) |
|---|---:|---:|---:|
| Recommended ask (USD) | $150,000 | $150,000 | $200,000 |
| Fee in INR | ₹127.5L | ₹127.5L | ₹170.0L |
| **Delivery cost (Base, INR)** | **₹110.4L** | **₹75.3L** | **₹250.3L** |
| Gross profit | ₹17.1L | ₹52.2L | **-₹80.3L** |
| Gross margin | ~13.4% | ~40.9% | **below cost** |
| **Net profit after tax (~25.17%)** | **~₹12.8L (~$15.1K)** | **~₹39.1L (~$46.0K)** | **loss** |

**Headline:** Lab and Premiere carry healthy margin at $150K each. Internship at $200K/year is **below delivery cost** (~$294K); flag for Rabia before client send. Dell or EMG selects **one** program, not a bundled tier.

---

## 2. Program 1: Screenwriters Residency Lab

Source: `../00-intake/Screenwriters lab - BUDGET - Sheet1-3.pdf`

| Cost line | INR |
|---|---:|
| Accommodation & venue | 14,00,000 |
| Travel | 12,00,000 |
| Mentor honorariums | 10,00,000 |
| Program team | 19,50,000 |
| Design, content & outreach | 11,50,000 |
| Legal, registration & IP | 1,50,000 |
| Subtotal | 68,50,000 |
| Contingency (10%) | 6,85,000 |
| WIF India fees | 35,00,000 |
| **Lab total** | **₹110.4L** |

One-time program cycle. 16 pax, 3-day residential lab, Kolkata/Darjeeling. Accommodation & venue reduced from ₹20.5L (5-day) to reflect the shorter 3-day format; pending updated lab budget sheet.

---

## 3. Program 2: Premiere Program (draft)

Source: `../04-specs/event-spec.md` (draft assumptions)

| Cost line | INR |
|---|---:|
| Program team (12 months) | 18,00,000 |
| Premiere events (6 x high-impact format) | 24,00,000 |
| Marketing, PR, social | 12,00,000 |
| Premiere production / venue coordination | 10,00,000 |
| Filmmaker travel / hospitality | 6,00,000 |
| Legal / rights coordination | 3,00,000 |
| Subtotal | 73,00,000 |
| Contingency (10%) | 7,30,000 |
| WIF India program fee | 15,00,000 |
| **Premiere Year 1 total** | **₹75.3L (~₹75L)** |

Annual recurring if program continues.

---

## 4. Program 3: Internship Program (annual)

Source: `../00-intake/Proposal for Kamla Mills Foundation - Internal .docx`

| Cost line | INR |
|---|---:|
| Stipends (35 x 6mo x ₹25K) | 52,50,000 |
| Support team (HR, placement) | 60,00,000 |
| Host mentor honorariums | 30,00,000 |
| Travel & accommodation | 40,00,000 |
| Program management + M&E | 35,00,000 |
| Closing ceremony | 10,00,000 |
| Subtotal | 2,27,50,000 |
| Contingency (10%) | 22,75,000 |
| WIF India program fee | 52,50,000 (implied in ₹2.5 Cr total) |
| **Internship annual total** | **₹250.3L (~₹2.5 Cr)** |

30–35 interns per year at $200K/year ask.

---

## 5. Per-program pricing summary

| Program | Delivery cost (INR) | Delivery cost (USD) | Ask (USD) | Margin |
|---|---:|---:|---:|---|
| Screenwriters Lab | ₹110.4L | ~$130K | $150,000 | ~15% above cost |
| Premiere | ₹75.3L | ~$88K | $150,000 | ~70% above cost |
| Internship (annual) | ₹250.3L | ~$294K | $200,000 | **below cost** |

---

## 6. Income model by program

| | Lab | Premiere | Internship |
|---|---:|---:|---:|
| Fee (USD) | $150,000 | $150,000 | $200,000 |
| Fee (INR) | ₹127.5L | ₹127.5L | ₹170.0L |
| Less: delivery cost | ₹110.4L | ₹75.3L | ₹250.3L |
| Gross profit | ₹17.1L | ₹52.2L | -₹80.3L |
| Tax (~25.17%) | ₹4.3L | ₹13.1L | n/a |
| Net profit | ₹12.8L | ₹39.1L | loss |

---

## 7. Multi-year view (Internship extension)

If Dell commits to multi-year internship:

| Year | Scope | Annual Dell ask (USD) |
|---|---|---:|
| Year 1 | Internship (30–35 women) | $200,000 |
| Year 2 | Internship (30–35 women) | $200,000 |
| Year 3 | Internship (30–35 women) | $200,000 |
| **3-year total** | | **$600,000** |

Lab and Premiere are priced independently if Dell sponsors those programs in other years.

---

## 8. Sensitivity / overrun risks

- **Lab:** Mentor travel upgrades, venue changes (Darjeeling logistics)
- **Premiere:** Venue and production costs could rise 20-30% for high-impact format
- **Internship:** Placement shortfall may require extended search costs; stipend pressure if cohort grows beyond 35
- **FX:** INR depreciation increases WIF cost base on USD-priced fees

---

## 9. Recommendation

- Present **three independent options**; Dell or EMG selects one
- **Lab ($150K)** and **Premiere ($150K)** are both viable at healthy margin
- **Internship ($200K/year)** is below delivery cost; use only with Rabia sign-off or scope adjustment
- Do not bundle programs into tier packages ($265K / $595K model retired)
